...
async def create_project_auto_endpoint(request: Request):
    ...
    try:
        ...
        print(f"[devps] Creating GitHub repo: {project_name}", file=sys.stderr)
        repo_url = github_ops.create_repo(project_name, github_token, f"Auto-generated by devps")
        print(f"[devps] Repo created: {repo_url}", file=sys.stderr)

        project_dir = Path(config.PROJECTS_DIR) / project_name
        print(f"[devps] Cloning repo to {project_dir}", file=sys.stderr)
        docker_ops.clone_or_update(repo_url, project_dir, "main")

        print(f"[devps] Initializing docker-compose files", file=sys.stderr)
        github_ops.init_repo_with_compose(project_dir, project_name)

        print(f"[devps] Creating deploy request", file=sys.stderr)
        ...
        print(f"[devps] Deploying project", file=sys.stderr)
        deploy_project(project_name, deploy_req)

        print(f"[devps] Setting project owner", file=sys.stderr)
        with connect() as conn:
            conn.execute(
        ...
        print(f"[devps] Project {project_name} created successfully", file=sys.stderr)
        return JSONResponse({
            "success": True,
        ...
        print(f"[devps] ERROR: {error_msg}", file=sys.stderr)
